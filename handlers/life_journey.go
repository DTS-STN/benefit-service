...
func (h *Handler) LifeJourneys(c echo.Context) error {
	...
	if lifeJourneyRequest.Id != "" {
		if lifeJourneyResponse.LifeJourney, err = lifejourneys.Service.GetByID(lifeJourneyRequest.Id); err != nil {
			c.Logger().Error(err)
			return c.JSON(http.StatusBadRequest, lifeJourneyResponse)
		}
	...
	lifeJourneyResponse.LifeJourneyList, err = lifejourneys.Service.GetAll()
...
func (h *Handler) LifeJourneyBenefits(c echo.Context) error {
	...
	for _, benId := range lifeJourney.RelatedBenefits {
		benefit, err := benefits.Service.GetByID(benId)
		if err != nil {
			c.Logger().Error(err)
